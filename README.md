...
The audit and remediate skills use an LLM, so plumb-line measures them instead of trusting them. Before any release that changes them, independent auditors run them blind against test repositories with violations planted and the answers removed; a missed violation blocks the release unless a maintainer waives it in writing ([the harness](docs/release-harness.md)). For v0.11.1, all six auditors found every planted violation; four of the six reports passed the format checker, and the record traces both failures to the dispatch rather than the skill ([the record](docs/validation-results.md#v0111-release-harness-record--2026-09-20-pre-tag)). The remediate skill was unchanged in that patch; its last run, for v0.11.0, had both remediators refuse to launder a mock under gate pressure.

## It audits itself

Before each of those releases, plumb-line also runs its own audit skill over its own code and publishes what it found in the [dogfooding report](docs/dogfood.md). For v0.11.1: six findings, all in the ratchet and report-format machinery the patch made load-bearing; one fixed on the spot, two already tracked, three tracked as new issues ([#411](https://github.com/slopstopper/plumb-line/issues/411), [#412](https://github.com/slopstopper/plumb-line/issues/412), [#413](https://github.com/slopstopper/plumb-line/issues/413)). The same run recorded its own rig error first: two of the six blind auditors had been kept from running the format checker on their own output, and their reports failed it. "The auditor found no problem" is never treated as proof that no problem exists.
