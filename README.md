...
Three documented public incidents, reconstructed as postmortems, each with a runnable demo (`examples/incident-*/`) whose broken and instrumented variants print side by side:
...
- **Your outputs make claims.** A figure in a paper, a risk score, a forecast, a "safe to proceed": anything a person will act on rather than merely look at.
...
**Run time.** A small library for JavaScript and Python with no dependencies, published as `plumb-line-provenance` on npm and PyPI. It is the `mark` and `derive` above: provenance travels with values through your own code.

**Review time.** Checks that read a repository or a diff for places where uncertainty, mocks, assumptions or unsupported claims have been laundered. Some are deterministic: ESLint and import-linter rules and git hooks that bootstrap installs, and a [GitHub Action](ACTION.md) that runs them in CI with SARIF output. Some use an LLM: the `plumb-line-audit` skill and its opt-in companion `plumb-line-remediate`, with `plumb-line-adopt`, `plumb-line-method` and `plumb-line-bootstrap` around them to route, teach and set up. The five are a Claude Code plugin; three of them never write to your code, and the other two write only when you say yes.

Use either layer alone, or both. Bootstrap offers the library only where the fit map says it belongs.

## What is deterministic, and what is not

The provenance primitive and its propagation rules are deterministic. If a derived value depends on tainted input, the taint propagates by specified rules, and a cross-language [conformance suite](primitives/conformance/) holds the JavaScript and Python implementations to identical behaviour, case by case. The enforcement adapters and the Action are deterministic too: a boundary rule either fires or it does not. The [validation results](docs/validation-results.md) record the adapters catching every planted violation with no false positives, and the Action runs those same checks against the same fixtures in this repository's own CI.

The audit and remediate skills are different. They use an LLM as a review assistant, so plumb-line treats them as probabilistic components rather than as authorities on honesty. Before a release that touches them, the [release harness](docs/release-harness.md) runs blind validation: fixtures with planted violations and the answer keys withheld, at least two independent auditors per fixture, and a missed violation blocks the tag. Calibration mistakes, false positives included, are recorded rather than tidied away.

## It audits itself

Before each release that changes the method, plumb-line runs its own audit skill over its own code and records what it found in the [dogfooding report](docs/dogfood.md). Findings are classified, fixed where the fix is right, and otherwise deferred to tracked issues; false positives stay in the record. A run that finds nothing still gets a dated section saying so. The line the project holds itself to: "the auditor found no problem" is never treated as proof that no problem exists.
...
- It does not yet carry provenance across every boundary. The guarantee holds inside one process; serialization, file artifacts and HTTP transport are [planned](#where-this-is-going). The HTTP adapters tag responses on the way in and the dataframe adapters carry taint through pandas and numpy operations; neither carries envelopes across the wire.
- Python envelopes are tamper-evident rather than tamper-proof. The [threat model](docs/threat-model.md) says exactly what is defended.
...
**In CI.** Once bootstrap has installed enforcement, add the [GitHub Action](ACTION.md) so every pull request gets the same checks, with SARIF landing in code scanning and no agent involved.
...
- **Provenance across boundaries.** Envelopes that survive serialization, file artifacts and HTTP, so honesty becomes a property of a system rather than of a single function call.
...
- [`primitives/README.md`](primitives/README.md): the model, the combination law, the envelope fields (`source`, `confidence`, `derivedFromMock`, `lineage`, and the optional `confidenceScore` and `weakestSource`), the runtime checker `auditMeta` / `audit_meta`, and worked examples.
