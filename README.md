...
The audit and remediate skills use an LLM, so plumb-line measures them instead of trusting them. Before any release that changes them, independent auditors run them blind against test repositories with violations planted and the answers removed; a missed violation blocks the release unless a maintainer waives it in writing ([the harness](docs/release-harness.md)). For v0.11.2, all six auditors found every planted violation, and all six reports pass the format checker. All six also failed it on the first try for the same reason: the checker could not read a glossary written inside a code fence, the form the skill's own example uses. That bug, which the v0.11.1 record had put down to the skill's output, was fixed before the tag ([the record](docs/validation-results.md#v0112-release-harness-record--2026-09-25-pre-tag)). The remediate skill was unchanged in that patch; its last run, for v0.11.0, had both remediators refuse to launder a mock under gate pressure.

## It audits itself

Before each of those releases, plumb-line also runs its own audit skill over its own code and publishes what it found in the [dogfooding report](docs/dogfood.md). For v0.11.2: five findings, one of them a violation (the audit skill still described its report as four parts in three places after the patch made it five); two fixed on the spot, three tracked as new issues ([#432](https://github.com/slopstopper/plumb-line/issues/432), [#433](https://github.com/slopstopper/plumb-line/issues/433), [#434](https://github.com/slopstopper/plumb-line/issues/434)). The three found by the previous release's run were all fixed in this one. "The auditor found no problem" is never treated as proof that no problem exists.
