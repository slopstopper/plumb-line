...
- **A v3 audit report must now carry the omission-pass table**
  ...
  with one column per question (a header may go on to inline-name its
  principle) and every row the header's width. A scope with no
  ...
  current checker unless it has the table. The committed README exemplar was
