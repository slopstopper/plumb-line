...
- **Contributor and component docs match the code again**
  ...
  - `DEVELOPMENT.md` no longer states test counts that were four to five times
  ...
  - SPEC, PARITY, the primitives README, the adapter contract, `docs/api.md`
    and the threat model each had a wrong cross-reference, count or scope
    claim; the Scorecard and coverage numbers are re-measured.
...
- **ADR-0018 declares plumb-line's own source-truth layer and
  ...
  on this repository in full. Drafted for the owner's review and accepted
  before merge.
...
- **The Python HTTP adapter module is now `http_adapter.py`**
  ...
  **Installed users need no change at run time:** `plumb_line_provenance.http`
  ...
  and the docs now show the new name. The alias is a runtime `sys.modules`
  entry, so static tools (pyright/Pylance) cannot resolve the old path;
  import `http_adapter` to keep them happy. **Flat-copy users** import `http_adapter` instead of loading
...
- **A v3 audit report must now carry the omission-pass table**
  ([#411](https://github.com/slopstopper/plumb-line/issues/411)). The audit
  skill always called the table REQUIRED, but `check_report_format` never
  checked it, so a report with no table at all could claim
  `format-validation: clean`. Checker v3 requires a table headed
  `| Output | Provenance | Confidence | Lineage | Contract | Null-expressible | Baseline |`:
  those words, in that order, one column per question (a header may go on
  to inline-name its principle), and every row the header's width with no
  blank cell. A scope with no
  output-producing units writes `No output-producing units in scope.` in
  its place. The skill now gives the literal header row. `report-format`
  stays v3. **Stricter:** a v3 report saved before this release fails the
  current checker unless it has the table; fed to `plumb-line-remediate`,
  such a report now draws checker violations (remediate warns and asks, it
  does not refuse). The committed README exemplar was
  regenerated by a fresh blind audit under the updated skill.

### Fixed
- `check_report_format` reads a principle glossary written inside a ```
  fence, the form the audit skill's own example uses. Code-span masking ate
  two of the fence's three backticks, the soft-wrap join then fused the
  glossary into one line between the leftovers, and a second mask blanked
  it, so every glossary-only principle read as "cited but not in the
  glossary". All six blind auditors in the v0.11.2 release harness hit this
  and had to unfence the glossary to pass. Stored reports that already
  passed still pass.
...
- `node primitives/conformance/report.mjs`, the self-certification gate and
  ...
  `primitives/conformance/run-cases.mjs`, which also fails any case field or
  top-level case kind it does not interpret; the two Python runners carry
  the same guards.
- `scripts/trigger_check.py` records the probe settings that change
  ...
  or turn cap was, and `--timeout` must be positive so the harness never
  writes a record its validator refuses. No v1 records are committed in this
  repository. The docs
