"""
engine/aggregate.py
Pure aggregation logic. Reads threshold prior from injected config — no hardcoded weights.
...
def aggregate_records(records, config):
    ...
    The signal threshold comes exclusively from the injected config argument.
    ...
    Returns:
        dict with keys:
            ...
            result          - mean if signal detected, else None (null result is valid)
            provenance      - str describing how the result was derived
            confidence      - float 0–1
            weights_version - str, config version used
    """
    if not records:
        return {
            "mean": None,
    ...
    return {
        ...
        "provenance": (
            ...
            f"threshold={threshold} (config v{config['version']}); "
