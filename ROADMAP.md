...
- **v0.9.0 — The front door** (distribution) · *created 2026-08-14, taking the
  ...
  substance of GH #252), the README/positioning pass (GH #258), an executable
  guard so fit-map snippets cannot rot (GH #262), and the Node floor resolution
  ...
  decided and recorded on the issue. Deliberately small: it exists so the
...
- **Python:** `requests`/`httpx` adapter — responses tagged by status + cache
  ...
  See [ADR-0012](docs/adr/0012-ecosystem-adapters-optional-deps-and-mapping.md).
...
**Priority: high** · Milestone: v0.11.0 · GitHub: #117 · **shipped**

Principle 9 (golden baseline + explain-the-drift) shipped in v0.11.0 as a
library + inspection CLI; cross-step causality, structural value diff and
float tolerance are `not-implemented`. The CLI pins an output *with its
lineage*, diffs runs, refuses silent drift, and demands a recorded one-line
explanation on update.
