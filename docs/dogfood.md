...
| `scripts/trigger_check.py` (threshold) | 0.5 pass threshold is a judgment call — named constant, but not injectable and not recorded in the results it judges | P5 — Injectable priors | **deferred** → [#317](https://github.com/slopstopper/plumb-line/issues/317); **resolved 2026-09-10**: `--threshold` stamped into the results, `results-format: v1`, `--validate` re-derives every verdict |
| `scripts/trigger_check.py` (results JSON) | durable measurement record with no format-version key or validator, unlike the sibling stored shapes | P7 — Contracted outputs | **deferred** → [#317](https://github.com/slopstopper/plumb-line/issues/317); **resolved 2026-09-10**: `--threshold` stamped into the results, `results-format: v1`, `--validate` re-derives every verdict |
...
| `scripts/trigger_check.py` (`build_payload`) | results record omits `--timeout` and the hard-coded max-turns; `--validate` proves self-consistency, not the reproducibility the docs claim | P8 — State-first lineage | **deferred** → [#400](https://github.com/slopstopper/plumb-line/issues/400); **resolved 2026-09-25**: `results-format: v2` records `probe: {timeout_s, max_turns}`, `--validate` refuses v1, docs no longer claim rates reproduce |
...
| `skills/plumb-line-audit/SKILL.md` | [needs-review] a new REQUIRED part under an unchanged `report-format: v3` makes v3 name two shapes, and the saved `format-validation:` line does not record which checker version passed it | P7 — Contracted outputs | **deferred** → [#432](https://github.com/slopstopper/plumb-line/issues/432); **fixed for v0.11.3 (PR #440; ships with the tag)** (owner chose the checker-versioned verdict line): `format-validation:` stamps `vN`; checker v4 rejects a future stamp and notes an older or missing one |
| `primitives/conformance/report.mjs` | [needs-review] the CONFORMANT verdict, badge and `--json` do not record which case table earned them; `cases.json`'s `version` is skipped unvalidated | P8 — State-first lineage | **deferred** → [#433](https://github.com/slopstopper/plumb-line/issues/433); **fixed for v0.11.3 (PR #440; ships with the tag)**: the verdict prints and emits the case-table version, sha256 and per-kind counts; all runners fail an unmodelled version |
...
| `AGENTS.md` | [needs-review, advisory] no source-truth layer or layer map is declared for the repo itself, so P1/P2 can only be checked partially | P6 — Maturity vocabulary | **deferred** → [#434](https://github.com/slopstopper/plumb-line/issues/434); **fixed for v0.11.3 (PR #440; ships with the tag)**: ADR-0018, accepted by the owner |
...
| Path | Issue | Principle | Resolution |
| ---- | ----- | --------- | ---------- |
| `adapters/python/hooks/branch_guard.py` (new CLI) and JS twin | an unset `PLUMBLINE_BRANCH` reads as "not protected": the guard exits 0 silently, treating an inconclusive result as a pass | spine — null-result expressibility | **deferred** → [#449](https://github.com/slopstopper/plumb-line/issues/449); **resolved 2026-09-26**: both twins block a code edit when the branch is unset or empty; a docs-allowlisted edit stays allowed (coupled with the next row; stricter for adopters) |
| `skills/plumb-line-bootstrap/SKILL.md` (hook contract) | [needs-review] "they work directly as commit hooks": direct wiring supplies neither the stdin nor the branch, so the branch guard never blocks | P6 — Maturity vocabulary | **deferred** → [#449](https://github.com/slopstopper/plumb-line/issues/449); **resolved 2026-09-26**: the claim is gone; the skill says git supplies neither input, shows how to set the branch in a Claude Code hook, and points at the planned wrapper (#464) |
...
## v0.11.4 dogfood self-audit — 2026-09-26

Scope: the method-surface diff v0.11.3..`3e9bb9a` (23 files). Report format
v3, validated with `scripts/check_report_format.py v4` — clean. Coverage:
23/23 files read at their changed hunks and enclosing units; the hook
sources, the four runners' case loops and the three case tables' key sets
...
**8 findings: 1 violation, 7 needs-review.**
...
| `adapters/adapter-contract.md` (hook I/O) | [needs-review] v0.11.4 changed what `Contract version: 1` promises in practice without moving it, and the twins' parity rests on hand-mirrored tests that already differ | P7 — Contracted outputs | **deferred** → [#475](https://github.com/slopstopper/plumb-line/issues/475) (a shared hook case table; the version question is recorded there for the owner) |
| `adapters/python/hooks/branch_guard.py` and JS twin | [needs-review] the twins decode stdin differently (Python by locale, strict; JS lossy UTF-8); every divergent case fails closed or allows a path genuinely under the allowlist | P7 — Contracted outputs | **deferred** → [#475](https://github.com/slopstopper/plumb-line/issues/475) |
| `primitives/conformance/README.md` | [needs-review] said a change to either table fails until runners learn it; the #441 guards catch a new field, kind or version, while a removed field or a changed value passes | P6 — Maturity vocabulary | **fixed in place** — the sentence now says what the guards catch |
| `adapters/js/hooks/pre-commit-gate.mjs` and Python twin (`decide`) | [needs-review] no runners reads as "all gates passed"; the CLI cannot reach it after #467 | spine — null-result expressibility | **deferred** → [#476](https://github.com/slopstopper/plumb-line/issues/476) (owner decision) |

### What was clean

The #441 guard models match exactly the fields each runner reads, and the
boolean-version fix holds in all three Python checks. PARITY.md's dated
note and the primitives README's container and version text match the
audit code in both languages. The provenance-lint README and SPEC §5b
wording on spawn coverage match the tests. The Python 3.14 classifier is
backed by the CI matrix.
