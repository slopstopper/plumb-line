...
<!-- constraints:begin -->
- Release version is **0.11.4**, and the three manifests must always agree: `primitives/js/package.json`, `primitives/python/pyproject.toml`, `.claude-plugin/plugin.json`. Bump only via `node scripts/bump-version.mjs <version>`.
...
- Python floor is **`requires-python = ">=3.11"`**; the CI matrix tests **3.11, 3.12, 3.13 and 3.14**.
...
tested at 3.11, Node floor 22 tested at 22.
